...
struct page* pop_page(struct page_bundle* bundle, struct memory_pool_ctx* ctx) {
	int retries = 4;
	do {
		// pr_info("pop_page: bundle %lx, stack=%lx, count %lu, global %lx", bundle, bundle->stack, bundle->count, &ctx->bundle_list);
		if (bundle->count > 0) {
			void* stack_page_virt = page_to_virt(bundle->stack);
...
/* 	int rc = 0; */
/* 	// Open does always */
/* 	struct memory_pool_ctx *ctx; */
/* 	ctx = kmalloc(sizeof(struct memory_pool_ctx), GFP_KERNEL); */
/* 	memset(ctx, 0, sizeof(struct memory_pool_ctx)); */
...
/* 	struct memory_pool_ctx *ctx = filp->private_data; */
/* 	/\* if (ctx->clone_of) *\/ */
...
void memory_pool_destroy(struct memory_pool_ctx* ctx) {
	...
	struct page* node = exmap_llist_del_all(&ctx->bundle_list);
	while (node) {
		struct page* stack = node;
		node = get_without_tag((struct page*) node->mapping);
